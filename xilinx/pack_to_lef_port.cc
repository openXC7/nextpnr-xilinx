...
int netlist_emit_clock_xdc(Netlist *n, const std::string &path, const std::string &spec)
{
    ...
    // parse "k=period,k=period"; order matters, first key matching a cell wins
    std::vector<std::pair<std::string, double>> pairs;
    {
        size_t i = 0;
        while (i <= spec.size()) {
            size_t c = spec.find(',', i);
            if (c == std::string::npos) c = spec.size();
            std::string kv = spec.substr(i, c - i);
            size_t eq = kv.find('=');
            if (eq != std::string::npos) {
                ...
                try {
                    if (!k.empty()) pairs.emplace_back(k, std::stod(trim(kv.substr(eq + 1))));
                } catch (...) {
                }
            }
            i = c + 1;
    ...
    if (pairs.empty()) {
        fprintf(stderr, "[clocks] TOPO_CLOCKS_XDC set but TOPO_CLOCK_PERIODS empty -- no "
                        "constraints emitted\n");
        return 0;
    }
    JPtr mods = n->root->member("modules");
    if (!mods || mods->k != JVal::OBJ) return -1;
    JPtr mj = nullptr;
    size_t bestn = 0;
    for (auto &kv : mods->obj) {
        JPtr c = kv.second->member("cells");
        size_t cnt = (c && c->k == JVal::OBJ) ? c->obj.size() : 0;
        if (!mj || cnt > bestn) { mj = kv.second; bestn = cnt; }
    }

    // driver bit -> "name" or "name[i]".  Index over the RAW bits array
    // INCLUDING constant entries: that is how nextpnr numbers a bus member.
    // Filtering constants out first shifts every later index -- on ethmin
    // eth.i_pcs_pma._61 has constants at 32/38/39, so the userclk BUFG's bit 43
    // came out as [40] and the create_clock silently matched nothing.
    std::unordered_map<int, std::string> bitname;
    JPtr nn = mj->member("netnames");
    if (nn && nn->k == JVal::OBJ)
        for (auto &net : nn->obj) {
            JPtr bl = net.second->member("bits");
            if (!bl || bl->k != JVal::ARR) continue;
            size_t w = bl->arr.size();
            for (size_t i = 0; i < w; i++) {
                if (bl->arr[i]->k != JVal::NUM) continue;
                int b = int(bl->arr[i]->num);
                if (bitname.count(b)) continue;
                bitname[b] = w > 1 ? net.first + "[" + std::to_string(i) + "]" : net.first;
            }
        }

    std::string out;
    out += "# GENERATED by the place_lef transplant (TOPO_CLOCK_PERIODS) -- do not edit.\n";
    out += "# nextpnr can only create_clock on PORTS; these are BUFG outputs\n";
    out += "# it cannot derive, so without them every clock falls back to --freq.\n";
    int emitted = 0;
    ...
    if (cells && cells->k == JVal::OBJ)
        for (auto &c : cells->obj) {
            ...
            if (t != "BUFG" && t != "BUFGCTRL") continue;
            const std::pair<std::string, double> *hit = nullptr;
            for (auto &kv : pairs)
                if (c.first.find(kv.first) != std::string::npos) { hit = &kv; break; }
            if (!hit) continue;
            JPtr conns = c.second->member("connections");
            if (!conns || conns->k != JVal::OBJ) continue;
            std::vector<int> ob = bits_of(conns->member("O"));
            if (ob.empty()) continue;
            auto bn = bitname.find(ob[0]);
            if (bn == bitname.end()) {
                fprintf(stderr,
                        "[clocks] WARNING %s: driver bit %d of %s has no netname -- NOT "
                        "constrained\n",
                        hit->first.c_str(), ob[0], c.first.c_str());
                continue;
            }
            char line[1024];
            snprintf(line, sizeof(line), "create_clock -period %.3f -name %s [get_nets {%s}]\n",
                     hit->second, hit->first.c_str(), bn->second.c_str());
            out += line;
            fprintf(stderr, "[clocks] %-16s %6.3f ns  cell=%s  net=%s\n", hit->first.c_str(),
                    hit->second, c.first.c_str(), bn->second.c_str());
            matched.insert(hit->first);
            emitted++;
        }
    // Say so loudly: an unmatched key means that clock silently keeps the
    // --freq default, which is exactly the failure this exists to prevent.
    for (auto &kv : pairs)
        if (!matched.count(kv.first))
            fprintf(stderr,
                    "[clocks] WARNING no BUFG matched '%s' -- that clock keeps the --freq "
                    "default\n",
                    kv.first.c_str());
    std::ofstream f(path);
    if (!f) {
        fprintf(stderr, "[clocks] cannot write %s\n", path.c_str());
        return -1;
    }
    f << out;
    fprintf(stderr, "[clocks] wrote %d create_clock line(s) -> %s\n", emitted, path.c_str());
    return emitted;
}
