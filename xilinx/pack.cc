...
void XilinxPacker::pack_srls()
{
    ...
    for (auto cell : sorted(ctx->cells)) {
        ...
        if (orig_type == "SRL16E") {
            ...
            // A 5LUT-position SRL16E takes its data in via DI1 (the AI pin)
            // and outputs on O5; a 6LUT-position one uses DI2/O6.  A6 does not
            // exist on the 5LUT bel, so it may only be tied high in the 6LUT
            // case.  An absolute BEL attribute (imported Vivado placement) can
            // force the 5LUT position; otherwise follow whatever port mapping
            // the transform rules above produced.
            bool is5 = (ci->ports.count(id_O5) != 0) || (ci->ports.count(id_O6) == 0);
